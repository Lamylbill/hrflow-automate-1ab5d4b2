...
export function exportEmployeesToExcel(employees: Employee[]) {
  ...
  const headers = Array.from(allFields).map(field => field.split('_').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' '));
  const data = employees.map(employee => Array.from(allFields).map(field => {
